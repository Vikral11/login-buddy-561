...
export function TrustFooter({
  ...
  return (
    <div className={`grid ${layoutClassName ?? "grid-cols-1 sm:grid-cols-3"} gap-2 ${compact ? "sm:gap-1.5" : "sm:gap-3"}`}>
      {items.map((it) => {
        const Icon = it.icon;
        return (
          <div
            key={it.label}
            className={`flex h-full items-start gap-2 rounded-2xl border bg-white/80 ${compact ? "min-h-[82px] px-2.5 py-2.5" : "px-3.5 py-3.5"}`}
            style={{ borderColor: "#EEF0F5" }}
          >
            <div
              className={`mt-0.5 flex shrink-0 items-center justify-center rounded-lg ${compact ? "h-6 w-6" : "h-8 w-8"}`}
              style={{ background: it.bg }}
            >
              <Icon className={compact ? "h-3 w-3" : "h-4 w-4"} style={{ color: it.color }} />
            </div>
            <div className="min-w-0">
              <p className={`font-bold leading-tight text-[#0F172A] ${compact ? "text-[11px]" : "text-[12px]"}`}>{it.label}</p>

...
export const Route = createFileRoute("/_app/email-classification")({
  head: () => ({ meta: [{ title: "Email Classification — Agentic" }] }),
  component: EmailClassificationPage,
});
...
const CATEGORY_META: Record<
  Category,
  { icon: React.ComponentType<{ className?: string }>; color: string; ring: string; bg: string }
> = {
...
// Mock data — backend (FastAPI) will populate this after Gmail sync.
...
function ClassificationSummaryCard({ category, count, total }: { category: Category; count: number; total: number }) {
  const m = CATEGORY_META[category];
  const Icon = m.icon;
  const pct = total > 0 ? Math.round((count / total) * 100) : 0;
  return (
    <div className="rounded-xl border border-border bg-card/60 p-4 backdrop-blur transition-colors hover:border-primary/30">
...
function EmailClassificationPage() {
  ...
  const filtered = useMemo(() => {
    ...
    // Mock time-based sort using array order as proxy
    return sortDesc ? rows : [...rows].reverse();
  }, [query, categoryFilter, priorityFilter, sortDesc]);

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col gap-2">
        <div className="inline-flex w-fit items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-2.5 py-0.5 text-[11px] font-medium text-primary">
          <Sparkles className="h-3 w-3" /> AI-Powered
        </div>
        <h1 className="text-2xl font-bold tracking-tight lg:text-3xl">AI Email Intelligence</h1>
        <p className="text-sm text-muted-foreground">Automatically categorized and prioritized emails.</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
        <StatCard icon={Inbox} label="Total Emails" value={total} accent="bg-primary/10 text-primary" />
        <StatCard icon={AlertTriangle} label="High Priority" value={high} accent="bg-red-500/10 text-red-500" />
        <StatCard icon={TrendingUp} label="Medium Priority" value={medium} accent="bg-amber-500/10 text-amber-500" />
        <StatCard icon={Brain} label="Low Priority" value={low} accent="bg-muted text-muted-foreground" />
        <StatCard icon={Sparkles} label="Most Active" value={mostActive} accent="bg-emerald-500/10 text-emerald-500" />
      </div>

      {/* Classification overview cards */}
      <div>
        <h2 className="mb-3 text-sm font-semibold text-muted-foreground">Classification Overview</h2>
      ...
      {/* Classified emails table */}
      <div className="rounded-xl border border-border bg-card/60 p-4 backdrop-blur">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-base font-semibold">Recently Classified Emails</h2>
            <p className="text-xs text-muted-foreground">Generated by the AI classifier after Gmail sync.</p>
          </div>
      ...
      <div>
        <h2 className="mb-3 text-sm font-semibold text-muted-foreground">Category Breakdown</h2>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          {CATEGORIES.map((c) => {
            const m = CATEGORY_META[c];
            const Icon = m.icon;
            const list = EMAILS.filter((e) => e.category === c).slice(0, 3);
            const high = EMAILS.filter((e) => e.category === c && e.priority === "High").length;
            const med = EMAILS.filter((e) => e.category === c && e.priority === "Medium").length;
            const lo = EMAILS.filter((e) => e.category === c && e.priority === "Low").length;
            return (
              <div key={c} className="rounded-xl border border-border bg-card/60 p-4 backdrop-blur">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground">{counts[c]} emails</p>
                    </div>
                  </div>
                </div>
                <ul className="mt-3 space-y-1.5">
                  ...
                  {list.length === 0 && <li className="text-xs text-muted-foreground">No emails yet.</li>}
                </ul>
                <div className="mt-3 flex items-center gap-2 text-[11px] text-muted-foreground">
                  <span className="inline-flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-red-500" />{high}</span>
                  <span className="inline-flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-amber-500" />{med}</span>
                  <span className="inline-flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-muted-foreground" />{lo}</span>
